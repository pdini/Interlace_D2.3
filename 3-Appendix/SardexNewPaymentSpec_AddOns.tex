...
In that ASM example we assume that \ASM{SelectOpenDeltaDebtFor} is returning a list of open \textit{delta debt} entries from the persistence layer with $debtPos(delta)>0$. \ASM{SelectMinDelta} selects the delta debt of a respective list with a minimum timestamp given that all cleared debts having $debtPos(delta)=0$ are excluded.

$debtPos(delta)$ is reading or writing the pending amount for a particular debt (here: $delta$).
