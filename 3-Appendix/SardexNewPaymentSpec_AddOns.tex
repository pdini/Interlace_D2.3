...
\begin{asm}
	...
	\IF amount \not = 0 \THEN \mbox{  // no debt change when }amount=0\+	    
		\LET openDeltas = \ASM{SelectOpenDeltaDebtsFor}(to)\+
			\IF openDeltas \not = \emptyset \THEN \+
			   \ASM{ClearDebt}(openDeltas, amount) \dec\dec\dec\-
	\WHERE \+
		\ASM{ClearDebt}(openDeltas, amount) =\+	
			\LET Tmin = \ASM{SelectOldestDebt}(openDeltas) \mbox{  // NB. }Tmin \not = \UNDEF\\				\IF debtPos(Tmin) \ge amount \THEN\+
...
In this ASM example we assume that \ASM{SelectOpenDeltaDebtsFor} is returning a set of open \textit{deltaDebt} entries from the persistence layer where $debtPos$ of each $deltaDebt$ is bigger than $0$. \ASM{SelectOldestDebt} selects the $deltaDebt$ of a corresponding set with the minimum timestamp. Further, \ASM{SelectOldestDebt} returns \textbf{undef} if $openDeltas$ are empty or \textbf{undef}.
